...
RegisterNetEvent('DD-oxyrun:client:start', function ()
    QBCore.Functions.TriggerCallback("DD-oxyrun:server:getCops", function(enoughCops)
        if enoughCops >= Config.MinimumPolice then
            QBCore.Functions.TriggerCallback("DD-oxyrun:server:coolc",function(isCooldown)
				if isCooldown then
				...
				QBCore.Functions.Progressbar("search_register", Lang:t("progress.talking_to_boss"), 5000, false, true, {
		            ...
		            if not routesFull then
						TriggerServerEvent('DD-oxyrun:server:startr')
					end
				end, function() -- Cancel
					if Config.EnableAnimation then
						TriggerEvent('animations:client:EmoteCommandStart', {"c"})
...
RegisterNetEvent("DD-oxyrun:createOxyVehicle")
AddEventHandler("DD-oxyrun:createOxyVehicle", function(route)
	...
	exports['qb-target']:AddTargetEntity(oxyVeh, {
		options = {
			{
				type = "client",
				event = "DD-oxyrun:client:checkitem",
...
AddEventHandler("DD-oxyrun:deleteOxyVehicle", function(ped, veh, route)
...
RegisterNetEvent("DD-oxyrun:client:deliverPackage")
AddEventHandler("DD-oxyrun:client:deliverPackage", function()
		exports['qb-target']:RemoveTargetEntity(oxyVeh, Lang:t("target.handoff_package"))
		holdingBox = false
		TriggerEvent("DD-oxyrun:deleteOxyVehicle",oxyPed,oxyVeh, Routes)
		if currentCars <= deliveries then
			currentCars = currentCars + 1
			TriggerEvent('QBCore:Notify', 'Temiz iş, (' .. currentCars .. '/' .. deliveries .. ')')
		...
		TriggerEvent('animations:client:EmoteCommandStart', {"c"})
		TriggerServerEvent("DD-oxyrun:server:deliver")
		Delivered = true
		Wait(1000)
		Delivered = false
		if currentCars == deliveries then
			TriggerEvent("DD-oxyrun:client:endOxy")
			return
		end
		Wait(Config.MinTimeBetweenCars*1200, Config.MaxTimeBetweenCars*1200)
		TriggerEvent("DD-oxyrun:createOxyVehicle",Routes)
end)

RegisterNetEvent('DD-oxyrun:client:checkitem')
AddEventHandler('DD-oxyrun:client:checkitem', function()
    QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
        if hasItem then
            TriggerEvent('DD-oxyrun:client:deliverPackage')
        else
...
RegisterNetEvent("DD-oxyrun:client:hasPackage")
AddEventHandler("DD-oxyrun:client:hasPackage", function()
	local ped = PlayerPedId()

	QBCore.Functions.TriggerCallback('DD-oxyrun:server:hasPackage', function(hasItems)
		if hasItems then
...
RegisterNetEvent('DD-oxyrun:client:SetOccupied', function(route)
	Config.Routes[route].info.occupied = true
end)

RegisterNetEvent('DD-oxyrun:client:sendToOxy', function()
	...
	if trueCount == totalCount then
		...
		TriggerServerEvent('DD-oxyrun:server:SetOccupied', Routes)
		...
		TriggerServerEvent('DD-oxyrun:server:TakeMoney', src)
		local carRoute = Config.Routes[Routes]
	
		gettingBox = true
		gettingVehicle = true
		TriggerEvent('DD-oxyrun:client:email')
		routesFull = false
	end
end)
...
RegisterNetEvent('DD-oxyrun:client:getBox', function()
	if gettingBox then
		amountOfBox = deliveries

		QBCore.Functions.TriggerCallback('DD-oxyrun:server:canCarry', function(canCarry)
			if canCarry then
				if currentBoxes < amountOfBox then
					TriggerEvent("DD-oxyrun:client:hasPackage")
					...
					if currentBoxes == amountOfBox then
						...
						TriggerEvent("DD-oxyrun:client:startOxy")
						gettingBox = false
					end
				end
			end
...
RegisterNetEvent('DD-oxyrun:client:startOxy', function()
	local carRoute = Config.Routes[Routes]
...
RegisterNetEvent('DD-oxyrun:client:sendCar', function()
	if doingOxy and atOxy then
		TriggerServerEvent("DD-oxyrun:server:sendCar", Routes)
	end
end)

RegisterNetEvent('DD-oxyrun:client:endOxy', function()  
	...
	TriggerEvent('DD-oxyrun:client:endemail')
	end
		...
		DropZone:onPlayerInOut(function(isPointInside)
			if isPointInside then
				...
				if doingOxy and atOxy and not zoneEntered then
					TriggerEvent("DD-oxyrun:client:sendCar")
...
RegisterNetEvent('DD-oxyrun:client:email', function() 
	if Config.SendEmail then
...
function RunStart()
	Citizen.Wait(2000)
	TriggerServerEvent('DD-phone:server:sendNewMail', {
...
RegisterNetEvent('DD-oxyrun:client:endemail', function() 
	if Config.SendEmail then
	RunEnd()
	end
end)
